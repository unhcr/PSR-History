...
--
  ...
  --
    P_TEXT.SET_TEXT(nTXT_ID, 'DIMT', 'DESCR', nSEQ_NBR, psLANG_CODE, psDescription);
  --
    insert into DIMENSION_TYPES (CODE, DISPLAY_SEQ, ACTIVE_FLAG, TXT_ID)
...
--
  ...
  --
    ...
    into nTXT_ID, nVERSION_NBR, xDIMT_ROWID
  ...
  --
    ...
    then
      ...
      then P_TEXT.SET_TEXT(nTXT_ID, 'DIMT', 'DESCR', nSEQ_NBR, psLANG_CODE, psDescription);
      end if;
    --
...
--
  ...
  --
    if pnVERSION_NBR = nVERSION_NBR
    then
      delete from DIMENSION_TYPES where rowid = xDIMT_ROWID;
    --
...
--
  ...
  --
    if pnVERSION_NBR = nVERSION_NBR
    then
      P_TEXT.SET_TEXT(nTXT_ID, 'DIMT', psTXTT_CODE, pnSEQ_NBR, psLANG_CODE, psText);
    --
      update DIMENSION_TYPES
    ...
    PLS_UTILITY.START_MODULE
     (sVersion || '-' || sComponent || '.INSERT_DIMENSION_VALUE',
      psDIMT_CODE || '~' || psCODE || '~' || to_char(pnDISPLAY_SEQ) || '~' || psACTIVE_FLAG ||
        '~' || psLANG_CODE || '~' || to_char(length(psDescription)) || ':' || psDescription);
  --
    P_TEXT.SET_TEXT(nTXT_ID, 'DIM', 'DESCR', nSEQ_NBR, psLANG_CODE, psDescription);
  --
    insert into DIMENSION_VALUES (ID, DIMT_CODE, CODE, DISPLAY_SEQ, ACTIVE_FLAG, TXT_ID)
    values (DIM_SEQ.nextval, psDIMT_CODE, psCODE, pnDISPLAY_SEQ, psACTIVE_FLAG, nTXT_ID)
...
--
  ...
  --
    select TXT_ID, VERSION_NBR, rowid
    into nTXT_ID, nVERSION_NBR, xDIM_ROWID
  ...
  --
    ...
    then
      if psDescription is not null
      then P_TEXT.SET_TEXT(nTXT_ID, 'DIM', 'DESCR', nSEQ_NBR, psLANG_CODE, psDescription);
      end if;
    --
...
--
  ...
  --
    if pnVERSION_NBR = nVERSION_NBR
    then
      delete from DIMENSION_VALUES where rowid = xDIM_ROWID;
    --
      P_TEXT.DELETE_TEXT(nTXT_ID);
    else
      P_MESSAGE.DISPLAY_MESSAGE('DIM', 2, 'Dimension value has been updated by another user');
...
--
  ...
  --
    if pnVERSION_NBR = nVERSION_NBR
    then
      P_TEXT.SET_TEXT(nTXT_ID, 'DIM', psTXTT_CODE, pnSEQ_NBR, psLANG_CODE, psText);
    --
      update DIMENSION_VALUES
      set VERSION_NBR = VERSION_NBR + 1
      where rowid = xDIM_ROWID
