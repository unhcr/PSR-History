...
--
  for rCSV in
   (select LOCT_CODE || ',' ||
      ...
      to_char(ID) as LINE
    from
     (select LOC.LOCT_CODE, TXT.TEXT as NAME_EN, COU.ISO_COUNTRY_CODE,
        nvl(min(COU.START_DATE), LOC.START_DATE) as START_DATE, LOC.ID
      ...
      left outer join
       (select LOCR.LOC_ID_TO, LOCA.CHAR_VALUE as ISO_COUNTRY_CODE, LOC.START_DATE,
        ...
        and COU.SEQ_NBR = 1
      group by LOC.LOCT_CODE, TXT.TEXT, COU.ISO_COUNTRY_CODE, LOC.START_DATE, LOC.ID))
  loop
