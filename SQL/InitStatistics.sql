...
begin
  for rDEM in
   (with LOAD as
     (select COUNTRY_CODE, LOC_NAME, RCX_CODE, ACMT_DESCRIPTION, POPC_CODE, BASIS,
      ...
      from
       (select COUNTRY_CODE, regexp_replace(nvl(LOC_NAME, LOC_NAME_NEW), ' : .*$', '') LOC_NAME,
          RCX_CODE, ACMT_DESCRIPTION, POPC_CODE, BASIS,
      ...
      group by COUNTRY_CODE, LOC_NAME, RCX_CODE, ACMT_DESCRIPTION, POPC_CODE, BASIS)
    select
      case DEM.BASIS
        when 'C' then 'POCPOPC'
        when 'E' then 'POCPOPE'
      ...
      DEM.VALUE
    from
     (select COUNTRY_CODE, LOC_NAME, RCX_CODE, ACMT_DESCRIPTION, POPC_CODE, BASIS, substr(SEX_AGE, 1, 1) SEX_CODE, to_number(substr(SEX_AGE, 2)) AGE_FROM, VALUE
      from LOAD
      unpivot
    ...
    and HLOC.LOC_CODE_FROM = COU.CODE
    cross join
     (select ID
      from TIME_PERIODS
