...
begin
  for rDIM in
   (select DIMT_CODE, CODE, DESCRIPTION_EN, DISPLAY_SEQ, DESCRIPTION_FR, NOTES
    from STAGE.S_DIMENSION_VALUES)
  loop
    iCount := iCount + 1;
  --
    P_DIMENSION.INSERT_DIMENSION_VALUE(nDIM_ID, 'en', rDIM.DESCRIPTION_EN, rDIM.DIMT_CODE, rDIM.CODE, rDIM.DISPLAY_SEQ);
    nVERSION_NBR := 1;
  --
