...
with Q_DEMOGRAPHICS as
 (select ASR_YEAR, LOC_ID_ASYLUM_COUNTRY, LOC_ID_ASYLUM,
  ...
  from
   (select extract(year from STC.START_DATE) as ASR_YEAR, STC.STCT_CODE,
      STC.LOC_ID_ASYLUM_COUNTRY, STC.LOC_ID_ASYLUM,
...
--
Q_COUNTRIES_EN as
 (select LOC.ID, TXT1.TEXT as NAME_EN
  from T_LOCATIONS LOC
...
--
select ASR_YEAR, LOC_ID_ASYLUM_COUNTRY, LOC_NAME_ASYLUM_COUNTRY_EN,
  case
    when LOCT_CODE = 'ADMIN0' then LOCATION_TYPE_EN
    when LOC_COUNT > 1 then LOC_NAME_ASYLUM_EN || ' (' || LOCATION_TYPE_EN || ')'
    else LOC_NAME_ASYLUM_EN
  end as LOC_NAME_ASYLUM_EN,
  LOCT_CODE,
...
from
 (select DEM.ASR_YEAR, DEM.LOC_ID_ASYLUM_COUNTRY, COU.NAME_EN as LOC_NAME_ASYLUM_COUNTRY_EN,
    LOC.NAME_EN as LOC_NAME_ASYLUM_EN,
    LOC.LOCT_CODE, LOC.LOCATION_TYPE_EN,
    count(distinct DEM.LOC_ID_ASYLUM) over
      (partition by DEM.ASR_YEAR, DEM.LOC_ID_ASYLUM_COUNTRY, LOC.NAME_EN) as LOC_COUNT,
  ...
  inner join Q_COUNTRIES_EN COU
    on COU.ID = DEM.LOC_ID_ASYLUM_COUNTRY
  inner join Q_LOCATIONS_EN LOC
    on LOC.ID = DEM.LOC_ID_ASYLUM
  group by DEM.ASR_YEAR, DEM.LOC_ID_ASYLUM_COUNTRY, COU.NAME_EN,
    DEM.LOC_ID_ASYLUM, LOC.NAME_EN, LOC.LOCT_CODE, LOC.LOCATION_TYPE_EN);
