...
--
create or replace view V_ASR_POC_DETAILS_EN as
select ASR_YEAR,
...
--
create or replace view V_ASR_DEMOGRAPHICS_EN as
...
-- V_ASR_RSD_EN
--
create or replace view V_ASR_RSD_EN as
with Q_RSD as
...
-- FOCUS_LOCATIONS
--
create or replace view FOCUS_LOCATIONS
as
with Q_LOCATIONS as
 (select ID, NAME, LOCT_CODE, LOCTV_ID, START_DATE, END_DATE
  from
    (select LOC.ID, TXT.TEXT as NAME, LOC.LOCT_CODE, LOC.LOCTV_ID, LOC.START_DATE, LOC.END_DATE,
      row_number() over
       (partition by LOC.ID
        order by LANG.ACTIVE_FLAG desc, nvl(ULP.PREF_SEQ, LANG.DISPLAY_SEQ + 1e5)) as RANK
    from T_LOCATIONS LOC
  ...
  from
   (select LOCTV.ID, TXT.TEXT as DESCRIPTION,
    ...
    left outer join T_USER_LANGUAGE_PREFERENCES ULP
      ...
      and ULP.USERID = SYS_CONTEXT('PSR', 'USERID'))
  where RANK = 1)
select COUNTRY_CODE, COUNTRY_NAME, COUNTRY_START_DATE, COUNTRY_END_DATE, PSR_LOCATION_CODE,
  MSRP_SITE_CODE, LOCATION_NAME, LOCATION_TYPE, LOCATION_START_DATE, LOCATION_END_DATE
from
 (select LOCA1.CHAR_VALUE as COUNTRY_CODE, COU.NAME as COUNTRY_NAME,
    case
      when COU.START_DATE = TO_DATE('0001-01-01', 'YYYY-MM-DD') then null
      else COU.START_DATE
    end as COUNTRY_START_DATE,
    case
      when COU.END_DATE = TO_DATE('9999-12-31 23:59:59', 'YYYY-MM-DD HH24:MI:SS') then null
      else COU.END_DATE
    end as COUNTRY_END_DATE,
    LOC.ID as PSR_LOCATION_CODE,
    LOCA2.CHAR_VALUE as MSRP_SITE_CODE,
    LOC.NAME as LOCATION_NAME,
    nvl(LOCTV.DESCRIPTION, LOCT.DESCRIPTION) as LOCATION_TYPE,
    case
      when LOC.START_DATE = TO_DATE('0001-01-01', 'YYYY-MM-DD') then null
      else LOC.START_DATE
    end as LOCATION_START_DATE,
    case
      when LOC.END_DATE = TO_DATE('9999-12-31 23:59:59', 'YYYY-MM-DD HH24:MI:SS') then null
      else LOC.END_DATE
    end as LOCATION_END_DATE
  from Q_LOCATIONS COU
  inner join T_LOCATION_ATTRIBUTES LOCA1
    on LOCA1.LOC_ID = COU.ID
  ...
  left outer join T_LOCATION_ATTRIBUTES LOCA2
    on LOCA2.LOC_ID = LOC.ID
    and LOCA2.LOCAT_CODE = 'MSRPLOC'
  inner join Q_LOCATION_TYPES LOCT
    on LOCT.CODE = LOC.LOCT_CODE
  left outer join Q_LOCATION_TYPE_VARIANTS LOCTV
    on LOCTV.ID = LOC.LOCTV_ID
  where COU.LOCT_CODE = 'COUNTRY');

--
-- FOCUS_ORIGINS
--
create or replace view FOCUS_ORIGINS
as
with Q_LOCATIONS as
 (select ID, NAME, LOCT_CODE, START_DATE, END_DATE
  from
    (select LOC.ID, TXT.TEXT as NAME, LOC.LOCT_CODE, LOC.START_DATE, LOC.END_DATE,
       row_number() over
        (partition by LOC.ID
         order by LANG.ACTIVE_FLAG desc, nvl(ULP.PREF_SEQ, LANG.DISPLAY_SEQ + 1e5)) as RANK
    from T_LOCATIONS LOC
    ...
    left outer join T_USER_LANGUAGE_PREFERENCES ULP
      ...
      and ULP.USERID = SYS_CONTEXT('PSR', 'USERID'))
  where RANK = 1)
select ORIGIN_CODE, ORIGIN_NAME, ORIGIN_START_DATE, ORIGIN_END_DATE
from
 (select nvl(LOCA1.CHAR_VALUE, LOCA2.CHAR_VALUE) as ORIGIN_CODE, OGN.NAME as ORIGIN_NAME,
    case
      when OGN.START_DATE = to_date('0001-01-01', 'YYYY-MM-DD') then null
      else OGN.START_DATE
    end as ORIGIN_START_DATE,
    case
      when OGN.END_DATE = to_date('9999-12-31 23:59:59', 'YYYY-MM-DD HH24:MI:SS') then null
      else OGN.END_DATE
    end as ORIGIN_END_DATE
  from Q_LOCATIONS OGN
  left outer join T_LOCATION_ATTRIBUTES LOCA1
    on LOCA1.LOC_ID = OGN.ID
    and LOCA1.LOCAT_CODE = 'ISO3166A3'
  left outer join T_LOCATION_ATTRIBUTES LOCA2
    on LOCA2.LOC_ID = OGN.ID
    and LOCA2.LOCAT_CODE = 'HCRCC'
  where OGN.LOCT_CODE in ('COUNTRY', 'OTHORIGIN'));

--
-- FOCUS_STATISTICS
--
create or replace view FOCUS_STATISTICS
as
select STATSYEAR, ASYLUM_COUNTRY_CODE, MSRP_SITE_CODE, PSR_LOCATION_CODE, DISPLACEMENT_STATUS,
  ORIGIN_CODE, PPG_CODE, SEX_CODE, AGE_FROM, AGE_TO, VALUE
from
 (select extract(year from STC.START_DATE) as STATSYEAR,
    LOCA1.CHAR_VALUE as ASYLUM_COUNTRY_CODE,
    LOCA2.CHAR_VALUE as MSRP_SITE_CODE,
    STC.LOC_ID_ASYLUM as PSR_LOCATION_CODE,
    DST.CODE as DISPLACEMENT_STATUS,
    LOCA3.CHAR_VALUE as ORIGIN_CODE,
    PPG.PPG_CODE,
    STC.SEX_CODE,
    AGR.AGE_FROM,
    AGR.AGE_TO,
    STC.VALUE
  from
   (select START_DATE, DST_ID, LOC_ID_ASYLUM_COUNTRY, LOC_ID_ASYLUM, LOC_ID_ORIGIN_COUNTRY,
      PPG_ID, SEX_CODE, AGR_ID, sum(VALUE) as VALUE
    from T_STATISTICS
    where STCT_CODE in
     (select STCT_CODE
      from T_STATISTIC_TYPES_IN_GROUPS
      where STTG_CODE = 'DEMOGR')
    group by START_DATE, DST_ID, LOC_ID_ASYLUM_COUNTRY, LOC_ID_ASYLUM, LOC_ID_ORIGIN_COUNTRY,
      PPG_ID, SEX_CODE, AGR_ID) STC
  inner join T_LOCATION_ATTRIBUTES LOCA1
    on LOCA1.LOC_ID = STC.LOC_ID_ASYLUM_COUNTRY
    and LOCA1.LOCAT_CODE = 'ISO3166A3'
  left outer join T_LOCATION_ATTRIBUTES LOCA2
    on LOCA2.LOC_ID = STC.LOC_ID_ASYLUM
    and LOCA2.LOCAT_CODE = 'MSRPLOC'
  inner join T_DISPLACEMENT_STATUSES DST
    on DST.ID = STC.DST_ID
  left outer join T_LOCATION_ATTRIBUTES LOCA3
    on LOCA3.LOC_ID = STC.LOC_ID_ORIGIN_COUNTRY
    and LOCA3.LOCAT_CODE = 'ISO3166A3'
