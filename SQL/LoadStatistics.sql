...
begin
  for rDEM in
   (select
      ...
      DEM.VALUE
    from
     (select COUNTRY_CODE, LOC_NAME, RCX_CODE, ACMT_DESCRIPTION, DST_CODE, COUNTRY_CODE_ORIGIN, PPG, BASIS,
       ...
       (select COUNTRY_CODE, LOC_NAME, RCX_CODE, ACMT_DESCRIPTION, DST_CODE, COUNTRY_CODE_ORIGIN, PPG, BASIS,
        ...
        from
         (select COUNTRY_CODE, regexp_replace(nvl(LOC_NAME, LOC_NAME_NEW), ' : .*$', '') LOC_NAME,
            RCX_CODE, ACMT_DESCRIPTION, DST_CODE, upper(COUNTRY_CODE_ORIGIN) COUNTRY_CODE_ORIGIN, PPG, BASIS,
        ...
        group by COUNTRY_CODE, LOC_NAME, RCX_CODE, ACMT_DESCRIPTION, DST_CODE, COUNTRY_CODE_ORIGIN, PPG, BASIS)
      unpivot
    ...
    left outer join
     (select PPG1.ID, PPG1.DESCRIPTION, LOCR.LOC_ID_TO
      from POPULATION_PLANNING_GROUPS PPG1
      join LOCATION_RELATIONSHIPS LOCR
        on LOCR.LOC_ID_FROM = PPG1.LOC_ID
        and LOCR.LOCRT_CODE = 'COVER') PPG
    on PPG.LOC_ID_TO = COU.ID
    and PPG.DESCRIPTION = DEM.PPG
    cross join
     (select ID
      from T_TIME_PERIODS
