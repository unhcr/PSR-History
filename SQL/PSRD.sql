...
CREATE TABLE STATISTICS 
    ( 
     ...
     LOC_CODE NUMBER (9)  NOT NULL , 
     LOC_CODE_COUNTRY NUMBER (9)  NOT NULL , 
     COUNTRY_CODE VARCHAR2 (3)  NOT NULL , 
     LOC_CODE_ORIGIN NUMBER (9) , 
     LOC_CODE_COUNTRY_ORIGIN NUMBER (9) , 
...
FROM
  ...
  FROM
    ...
    SELECT LOC.CODE,
      ...
      LOC.TXT_ID,
      LOC.VERSION_NBR,
      -row_number() OVER (PARTITION BY LOC.CODE ORDER BY 1) ROW_NUMBER
    FROM LOCATIONS LOC
    INNER JOIN TEXT_ITEMS TXI
    ON TXI.TXT_ID     = LOC.TXT_ID
    AND TXI.TXTT_CODE = 'NAME'
...
SELECT LOC.CODE,
  LOC.NAME,
  LOC.LANG_CODE,
  LOC.COUNTRY_CODE AS ISO3166_ALPHA3_CODE,
  LOCA1.CHAR_VALUE AS ISO3166_ALPHA2_CODE,
  LOCA2.CHAR_VALUE AS ISO3166_NUMERIC_CODE,
  LOCA3.CHAR_VALUE AS UNHCR_COUNTRY_CODE,
  LOC.START_DATE,
  LOC.END_DATE,
  LOC.TXT_ID,
  LOC.VERSION_NBR
FROM L_LOCATIONS LOC
LEFT OUTER JOIN LOCATION_ATTRIBUTES LOCA1
ON LOCA1.LOC_CODE    = LOC.CODE
AND LOCA1.LOCAT_CODE = 'IS03166A2'
LEFT OUTER JOIN LOCATION_ATTRIBUTES LOCA2
ON LOCA2.LOC_CODE    = LOC.CODE
AND LOCA2.LOCAT_CODE = 'IS03166NUM'
...
FROM
  ...
  FROM
    ...
    SELECT LOCAT.CODE,
      ...
      LOCAT.TXT_ID,
      LOCAT.VERSION_NBR,
      -row_number() OVER (PARTITION BY LOCAT.CODE ORDER BY 1) ROW_NUMBER
    FROM LOCATION_ATTRIBUTE_TYPES LOCAT
    INNER JOIN TEXT_ITEMS TXI
    ON TXI.TXT_ID     = LOCAT.TXT_ID
    AND TXI.TXTT_CODE = 'DESCR'
...
FROM
  ...
  FROM
    ...
    SELECT LOCRT.CODE,
      ...
      LOCRT.TXT_ID,
      LOCRT.VERSION_NBR,
      -row_number() OVER (PARTITION BY LOCRT.CODE ORDER BY 1) ROW_NUMBER
    FROM LOCATION_RELATIONSHIP_TYPES LOCRT
    INNER JOIN TEXT_ITEMS TXI
    ON TXI.TXT_ID     = LOCRT.TXT_ID
    AND TXI.TXTT_CODE = 'DESCR'
...
FROM
  ...
  FROM
    ...
    SELECT LOCT.CODE,
      ...
      LOCT.TXT_ID,
      LOCT.VERSION_NBR,
      -row_number() OVER (PARTITION BY LOCT.CODE ORDER BY 1) ROW_NUMBER
    FROM LOCATION_TYPES LOCT
    INNER JOIN TEXT_ITEMS TXI
    ON TXI.TXT_ID     = LOCT.TXT_ID
    AND TXI.TXTT_CODE = 'DESCR'
