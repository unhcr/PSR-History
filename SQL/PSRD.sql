...
CREATE UNIQUE INDEX IX_PGR ON T_POPULATION_GROUPS 
    ( 
     nvl(DST_CODE, 'x'), nvl(LOC_ID_ASYLUM_COUNTRY, 0), nvl(LOC_ID_ORIGIN_COUNTRY, 0), nvl(LOC_ID_ASYLUM, 0), nvl(LOC_ID_ORIGIN, 0), nvl(DIM_ID1, 0), nvl(DIM_ID2, 0), nvl(DIM_ID3, 0), nvl(DIM_ID4, 0), nvl(DIM_ID5, 0), nvl(SEX_CODE, 'x'), nvl(AGR_ID, 0), nvl(PER_ID, 0), SEQ_NBR
    ) 
;
...
ALTER TABLE T_STATISTICS 
    ADD CONSTRAINT UK_STC UNIQUE ( STCT_CODE , DST_CODE , LOC_ID_ASYLUM_COUNTRY , LOC_ID_ASYLUM , LOC_ID_ORIGIN_COUNTRY , LOC_ID_ORIGIN , DIM_ID1 , DIM_ID2 , DIM_ID3 , DIM_ID4 , DIM_ID5 , PER_ID , SEX_CODE , AGR_ID , PGR_ID_SUBGROUP ) ;
...
ALTER TABLE T_POPULATION_GROUPS 
    ADD CONSTRAINT FK_PGR_PER FOREIGN KEY 
    ( 
     PER_ID
    ) 
...
ALTER TABLE T_STATISTICS 
    ADD CONSTRAINT FK_STC_PGR FOREIGN KEY 
    ( 
     PGR_ID_SUBGROUP
    ) 
    REFERENCES T_POPULATION_GROUPS 
...
FROM
  ...
  INNER JOIN TIME_PERIODS PER
  ON PER.ID = STC.PER_ID
  INNER JOIN TIME_PERIOD_TYPES PERT
  ON PERT.CODE = PER.PERT_CODE
...
SELECT LOC_NAME_ASYLUM_COUNTRY,
  ISO_COUNTRY_CODE,
  UNHCR_COUNTRY_CODE,
  LOC_ID_ORIGIN_COUNTRY,
  LOC_NAME_ORIGIN_COUNTRY,
  ...
  OTHERS_ASSISTED
FROM
  (SELECT STC.LOC_NAME_ASYLUM_COUNTRY,
    STC.ISO_COUNTRY_CODE,
    STC.UNHCR_COUNTRY_CODE,
    STC.LOC_ID_ORIGIN_COUNTRY,
    STC.LOC_NAME_ORIGIN_COUNTRY,
    STC.ISO_ORIGIN_CODE,
  ...
  GROUP BY STC.LOC_NAME_ASYLUM_COUNTRY,
    STC.ISO_COUNTRY_CODE,
    STC.UNHCR_COUNTRY_CODE,
    STC.LOC_ID_ORIGIN_COUNTRY,
    STC.LOC_NAME_ORIGIN_COUNTRY,
...
FROM
  ...
  FROM
    (SELECT STC.LOC_ID_ASYLUM_COUNTRY,
      STC.LOC_NAME_ASYLUM_COUNTRY,
      STC.ISO_COUNTRY_CODE,
      STC.UNHCR_COUNTRY_CODE,
      STC.LOC_ID_ORIGIN_COUNTRY,
      STC.LOC_NAME_ORIGIN_COUNTRY,
    ...
    GROUP BY STC.LOC_ID_ASYLUM_COUNTRY,
      STC.LOC_NAME_ASYLUM_COUNTRY,
      STC.ISO_COUNTRY_CODE,
      STC.UNHCR_COUNTRY_CODE,
      STC.LOC_ID_ORIGIN_COUNTRY,
...
FROM
  (SELECT Extract(YEAR FROM STC.PER_START_DATE) AS STATSYEAR,
    ...
    PGRA1.CHAR_VALUE AS SOURCE,
    PGRA2.CHAR_VALUE AS BASIS
  FROM STATISTICS STC
  LEFT OUTER JOIN T_POP_GROUP_ATTRIBUTES PGRA1
  ON PGRA1.PGRAT_CODE = 'SOURCE'
  LEFT OUTER JOIN T_POP_GROUP_ATTRIBUTES PGRA2
  ON PGRA2.PGRAT_CODE = 'BASIS'
