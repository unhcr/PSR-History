...
ALTER TABLE ORIGINS 
    ADD CONSTRAINT PK_OGN PRIMARY KEY ( CODE ) ;


ALTER TABLE ORIGINS 
    ADD CONSTRAINT UK_OGN_TXT_ID UNIQUE ( TXT_ID ) ;
...
ALTER TABLE STATISTICS 
    ADD CONSTRAINT UK_STC UNIQUE ( STCT_CODE , LOC_CODE , LOC_CODE_COUNTRY , LOC_CODE_ORIGIN , PPG_ID , POPC_CODE , PER_ID , SEX_CODE , AGR_ID , DIM_ID1 , DIM_ID2 , DIM_ID3 , DIM_ID4 , DIM_ID5 , OGN_CODE ) ;
...
ALTER TABLE ORIGINS 
    ADD CONSTRAINT FK_OGN_LOC FOREIGN KEY 
    ( 
     LOC_CODE
    ) 
    REFERENCES LOCATIONS 
...
ALTER TABLE STATISTICS 
    ADD CONSTRAINT FK_STC_OGN FOREIGN KEY 
    ( 
     OGN_CODE
    ) 
    REFERENCES ORIGINS 
...
CREATE OR REPLACE VIEW L_ORIGINS AS
SELECT CODE,
  NAME,
  LANG_CODE,
  LOC_CODE,
...
FROM
  (SELECT OGN.CODE,
    ...
    OGN.TXT_ID,
    OGN.VERSION_NBR,
    row_number() OVER (PARTITION BY OGN.CODE ORDER BY LANG.ACTIVE_FLAG DESC, NVL(ULP.PREF_SEQ, LANG.DISPLAY_SEQ + 1e5)) RANK
  FROM ORIGINS OGN
  INNER JOIN TEXT_ITEMS TXI
  ON TXI.TXT_ID     = OGN.TXT_ID
  AND TXI.TXTT_CODE = 'NAME'
  AND TXI.SEQ_NBR   = 1
  INNER JOIN LANGUAGES LANG
  ON LANG.CODE = TXI.LANG_CODE
...
SELECT STC.ID,
  ...
  STC.COUNTRY_CODE,
  LOCA1.CHAR_VALUE UNHCR_COUNTRY_CODE,
  STC.LOC_CODE_ORIGIN,
  LOC3.NAME LOC_NAME_ORIGIN,
  STC.OGN_CODE,
  OGN.NAME OGN_NAME,
  NVL(LOCA2.CHAR_VALUE, STC.OGN_CODE) UNHCR_OGN_CODE,
  STC.PPG_ID,
  STC.POPC_CODE,
...
ON LOCA1.LOC_CODE    = LOC2.CODE
AND LOCA1.LOCAT_CODE = 'UNHCRCC'
LEFT JOIN L_LOCATIONS LOC3
ON LOC3.CODE = STC.LOC_CODE_ORIGIN
LEFT JOIN L_ORIGINS OGN
ON OGN.CODE = STC.OGN_CODE
LEFT JOIN LOCATION_ATTRIBUTES LOCA2
ON LOCA2.LOC_CODE    = OGN.LOC_CODE
